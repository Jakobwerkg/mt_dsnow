...
cat("Initial score =", minimize_score(par_start, d_obs_fit_tibble, par_scale, verbose = TRUE), "\n")
...
cat(sprintf(
  ...
  fmt(best_unscaled["rho.max"]),  fmt(best_unscaled["rho.null"]), fmt(best_unscaled["c.ov"]),
  fmt(best_unscaled["k.ov"]),     fmt(best_unscaled["k"]),        fmt(best_unscaled["tau"]),
  fmt(best_unscaled["eta.null"])
))
...
fmt_tag <- function(x) gsub("-", "m", gsub("\\.", ".",
...
save_dir <- file.path(DATA_DIR, "R_opt_logs")
