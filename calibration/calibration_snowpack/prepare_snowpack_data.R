...
SUBSET_INPUT <- c(
    sp_all      = "data_18_all",
    sp_rg       = "data_rain_gauge",
    sp_b2000    = "station_below_2000",
    dyn_rho_max = "data_rain_gauge"
)
if (!SUBSET %in% names(SUBSET_INPUT)) {
    stop("Unknown DSNOW_SUBSET '", SUBSET, "'. Valid: ",
         paste(names(SUBSET_INPUT), collapse = ", "))
}

# Input: raw SNOWPACK .smet files for this subset
indir <- file.path(ROOT, "calibration", "calibration_data", "snowpack_data",
                   SUBSET_INPUT[[SUBSET]])

# Output: the subset's working directory, read by the calibration scripts
out_dir <- file.path(ROOT, "calibration", "optimisation_output", SUBSET, "data")
